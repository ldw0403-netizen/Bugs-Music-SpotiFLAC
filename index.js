...
function safeTracks(url) {
  try {
    return parseTracks(fetchHtml(url));
  } catch (error) {
    return [];
  }
}
...
function searchTracks(query) {
  if (!query || !String(query).trim()) return { success: true, tracks: [] };
  var tracks = safeTracks(BASE_URL + "/search/track?q=" + encodeURIComponent(String(query).trim()));
  return { success: true, tracks: tracks.map(makeTrackResult) };
}

function getTrack(trackId) {
  var id = String(trackId || "").replace("bugs:track:", "").trim();
  if (!id) return { success: false, error: "Track ID is missing" };
  var tracks = safeTracks(BASE_URL + "/track/" + id);
  var track = tracks.length ? tracks[0] : { id: id, name: "Bugs Track " + id, artists: "", album_name: "", cover_url: "" };
  return { success: true, track: makeTrackResult(track) };
}

function getHomeFeed() {
  var chart = safeTracks(BASE_URL + "/chart/track/realtime/total");
  var latest = safeTracks(BASE_URL + "/newest/track");
  var essential = safeTracks(BASE_URL + "/musicpd?tag_id=35486");
  var pd = safeTracks(BASE_URL + "/musicpd?order=list");
  var yearly = safeTracks(BASE_URL + "/chart/track/year");
  var radio = safeTracks(BASE_URL + "/radio");
  var genre = safeTracks(BASE_URL + "/genre");
  var sections = [];
  function add(uri, title, items) {
    if (items && items.length) sections.push(makeSection(uri, title, items));
  }
  add("bugs:chart", "벅스차트", chart);
  add("bugs:latest", "최신음악", latest);
  add("bugs:genre", "장르", genre);
  add("bugs:essential", "에센셜", essential);
  add("bugs:musicpd", "뮤직PD앨범", pd);
  add("bugs:yearly", "연도별", yearly);
  add("bugs:radio", "라디오", radio);
  return { success: true, greeting: "Bugs Music", sections: sections };
}
